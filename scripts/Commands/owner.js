...
ownerCommand.addArgument("player", async (player, target) => {
	...
	player.sendMessage(`§cYou are no longer owner! You can no longer bypass the anticheat!`)
	target.setDynamicProperty("isOwner", true)
	target.sendMessage(`§aYou have been promoted to owner! You can now bypass the entire anticheat and have access to every command!`)
})
